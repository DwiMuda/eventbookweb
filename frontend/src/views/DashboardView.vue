...
<template>
  <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
      <StatsCard label="Tingkat Kehadiran" :value="(stats?.checkinRate ?? 0) + '%'" :sub="`${stats?.checkedInBookings ?? 0} peserta hadir`" icon-bg="bg-emerald-50 text-emerald-600">
        <template #icon>
      ...
      <StatsCard label="Total Peserta" :value="stats?.totalBookings ?? '—'" sub="Booking terkonfirmasi" icon-bg="bg-brand-50 text-brand-600">
        <template #icon>
          <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
      ...
      <StatsCard label="Total Event" :value="stats?.totalEvents ?? '—'" sub="Event yang dikelola" icon-bg="bg-purple-50 text-purple-600">
        <template #icon>
          <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
